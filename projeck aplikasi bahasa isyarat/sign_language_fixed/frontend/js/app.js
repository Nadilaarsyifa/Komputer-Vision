...
function closeAllGuideImages() {
  ...
  const angkaVideo = document.getElementById("angka-guide-video");
  if (angkaVideo) {
    angkaVideo.pause();
    angkaVideo.removeAttribute("src");
    angkaVideo.load();
  }

  document.querySelectorAll(".huruf-btn, .angka-btn, .btn-kata-guide").forEach((b) => {
...
function showHurufGuide(huruf) {
  ...
  caption.textContent = "Gerakan untuk huruf: " + huruf;

  img.onerror = function () {
    this.onerror = null;
    this.src = "images/panduan_huruf_" + huruf + ".jpg";
  };
  img.src = "images/huruf_" + huruf + ".jpg";

  setActiveButton(".huruf-btn", "huruf", huruf);
}

function showAngkaGuide(angka) {
  const wrap = document.getElementById("img-wrap-angka");
  const videoEl = document.getElementById("angka-guide-video");
  ...
  caption.textContent = "Gerakan untuk angka: " + angka;

  videoEl.src = "videos/angka_" + angka + ".mp4";
  videoEl.load();
  videoEl.play().catch(() => {});

  setActiveButton(".angka-btn", "angka", angka);
}
...
function showKataGuide(kata) {
  ...
  const subcaption = document.getElementById("kata-guide-subcaption");
  ...
  caption.textContent = "Panduan Gerakan " + label;
  if (subcaption) subcaption.textContent = "Gerakan untuk kata: " + label;

  wrap.classList.remove("hidden");
  activeKataGuide = kata;
  setActiveButton(".btn-kata-guide", "kata", kata);
}

const kataSearch = document.getElementById("kata-search");
if (kataSearch) {
  kataSearch.addEventListener("input", () => {
    const keyword = kataSearch.value.trim().toLowerCase();
    let visibleCount = 0;

    document.querySelectorAll("#kata-list .btn-kata-guide").forEach((btn) => {
    ...
    const empty = document.getElementById("kata-empty");
